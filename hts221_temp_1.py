...
def readValue(i, value_type):
    with open(pathBuilder(i, value_type), "r") as in_temp_raw:
        flt_raw_input = in_temp_raw.readline()
        InValueRaw = float(flt_raw_input)
        return InValueRaw
        # DEBUG
...
def readTemp():
    while i < 4:
        ...
        if path.exists(pathBuilder(i, 'name')) and \
            ...
            sttemp = str(readValue(i, 'name').strip())
            # isfile = open(BASE_PATH + '%s/name' % i, "r")
